/**
 * Digital Hub training backend.
 * Runs in the browser, intercepts `/api/*`, and is replaced later by a real server
 * without changing student-facing API modules.
 */
...
function exposeMentorTools(server) {
  globalThis.__DIGITAL_HUB_MOCK__ = {
    reset() {
      server.db.emptyData();
      seedDatabase(server);
